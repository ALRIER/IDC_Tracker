...
# Full model — PMs can set all fields
...
# Full update — PMs and admin only
...
# Restricted update — ONLY these 4 fields for interviewers
...
def get_interviews(
    ...
    # PRIVACY RULE — interviewers only see their own rows
    if current_user.role == "interviewer":
        query = query.filter(Interview.interviewer == current_user.interviewer_name)

    # Optional filter by project
    if project_id:
        query = query.filter(Interview.project_id == project_id)

    return query.order_by(Interview.project_name, Interview.interviewee_name).all()
...
def get_interview(
    ...
    # Interviewers can only see their own rows
    if current_user.role == "interviewer":
...
def update_interview(
    ...
    # PRIVACY RULE — interviewers can only edit their own rows
    if current_user.role == "interviewer":
        if interview.interviewer != current_user.interviewer_name:
            raise HTTPException(status_code=403, detail="Access denied")

        # PRIVACY RULE — interviewers can only update these 4 fields
        # Any other field in the request is silently ignored
        allowed_fields = {"interview_status", "date_of_interview", "interview_quality", "interviewer_notes"}
...
@router.delete("/{interview_id}", dependencies=[Depends(require_role("pm", "admin"))])
