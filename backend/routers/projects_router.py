...
def get_projects(
    ...
    # Analysts need full audit visibility, including Closed projects.
    if current_user.role in ("admin", "pm", "analyst"):
...
def create_project(
    ...
    existing = (
        db.query(Project)
        .filter(Project.project_number == data.project_number)
        .first()
    )
...
@router.patch("/{project_id}", dependencies=[Depends(require_role("pm", "admin", "analyst"))])
def update_project(
    project_id: str,
    data: ProjectUpdate,
    db: Session = Depends(get_db)
