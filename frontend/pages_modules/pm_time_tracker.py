...
def build_unique_options(df, column_name, fallback_options=None):
    """
    Builds dropdown options from existing project data.
    Adds fallback options only as backup.
    """
    ...
    cleaned = sorted(list({x for x in combined if x and x.lower() not in ["nan", "none"]}))
